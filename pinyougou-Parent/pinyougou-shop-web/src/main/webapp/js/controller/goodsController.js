...
app.controller('goodsController' ,function($scope,$controller,$location,uploadService,itemCatService,typeTemplateService,goodsService){
    ...
    $scope.itemCatList=[];
    //便利goods中 商品級別  然後查詢tb_item_cat 中findOnebyId  取name賦值給goods. category1_id 變量
    //讀取商品列表
	...
	$scope.findOne=function(){
		...
		goodsService.findOne(id).success(
			function(response){
                ...
                //轉換sku中規格選項  就是將item中spec 轉為JSON對象
    ...
    $scope.$watch('entity.goods.typeTemplateId',function(newValue,oldValue){
        ...
        if(newValue!=undefined&&newValue!=-1){
            ...
            typeTemplateService.findOne(newValue).success(function(response){
				...
				///如果没有 ID，则加载模板中的扩展数据  否則findOne goods中數據會被覆蓋 查詢不到
	...
	//修改商品是否上架
	$scope.updateMarketable=function (marketabel) {
		goodsService.isMarketable($scope.selectIds,marketabel).success(function (response) {
            if(response.success){
                $scope.reloadList();//刷新列表
